...
const {
        ...
        localAddFile
        localRemoveFile
        localGetAllFiles } = useLocalIpfsStore(); // Used as a local cache to save bandwidth and improve performance
...
const { servers
        ...
        remotegetFileExtraProps } = useRemoteIpfsStore(); // Used to interact with the galacfetch IPFS nodes
...
```js
{
  status: undefined | 'idle' | 'loading' | 'error';
}
```
...
const editFileProps ={
 ...
 updateFile(cid:string, editFileProps:FilePropsEdit);
```
