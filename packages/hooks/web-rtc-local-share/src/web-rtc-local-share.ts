...
export const webRTCLocalShare = create<TWebRTCLocalShare>()(
  (set, get): TWebRTCLocalShare => ({
    ...
    createNewRTCPeerConnection: (ws, client) => {
      ...
      const newWebRTCConnectionInfo: WebRTCConnectionInfo = {
        localPeerId: ws.id as string,
    ...
    setupDataChannelEvents: (dataChannel, client) => {
      ...
      const ackHandlers = {} as Record<string, () => void>
      ...
      const handleCheckFileMessage = async (message: { cid: string }) => {
        arrayBufferStore = []
      ...
      const handleGetFileMessage = async (message: { cid: string }) => {
        arrayBufferStore = []
        ...
        if (isFile) {
          ...
          miniChunks.forEach((chunk: ArrayBuffer) => {
